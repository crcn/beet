...
module.exports = Structr({
    ...
    'all': function(callback)
    {
        ...
        this.beet.client.getAllProcessInfo(function(err, processes)
        {
            ...
            for(var i = processes.length; i--;)
            {
                ...
                if(proc.group != self.name)
                {
                    processes.splice(i, 1); 
                }
                else
                {
                    proc.name = proc.name.split('_').pop();
                }
            }   
    ...
    'start': function(name, callback)
    {
        ...
        this.beet.client.startProcess(this.name + ':' + name, function(err, result)
        {
             if(err)
            {
    ...
    'stop': function(name, callback)
    {
        ...
        this.beet.client.stopProcess(this.name + ':' + name, function(err, item)
        {
            if(err)
            {
